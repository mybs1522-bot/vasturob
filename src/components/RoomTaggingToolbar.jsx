    ...
    <div className="clean-card p-4 space-y-3 bg-white">
      ...
      <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-bold text-slate-900 text-xs font-heading">
              {isHi ? 'कमरे जोड़ें / स्थिति तय करें' : 'Add / Position Rooms'}
            </h3>
            <p className="text-[11px] text-slate-500">
              {isHi ? 'कमरा चुनने के लिए बटन पर टैप करें' : 'Tap buttons to add boxes onto slate'}
            </p>
          </div>
        </div>
        ...
        {placedRooms.length > 0 && (
          <button
            type="button"
            onClick={onClearAll}
            className="text-xs text-red-600 hover:bg-red-50 px-2 py-1 rounded-lg border border-red-200 transition-all font-semibold cursor-pointer"
          >
            {isHi ? `सभी हटाएं (${placedRooms.length})` : `Clear All (${placedRooms.length})`}
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
        {ROOM_TYPES.map((room) => {
          ...
          const displayName = isHi ? (room.name_hi || room.name) : room.name;
          ...
          return (
            <button
              ...
              className={`p-2 rounded-full border text-left flex items-center justify-between gap-1.5 transition-all cursor-pointer ${
                isSelected
      ...
      {placedRooms.length > 0 && (
        <div className="pt-2 border-t border-slate-100">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1.5">
            {isHi ? `चिह्नित कमरे (${placedRooms.length})` : `Placed Slate Room Boxes (${placedRooms.length})`}
          </span>
